...
class taskset:
    ...
    def __generate_taskset_from_json(self, json_file):
        ...
        with open(json_file, 'r') as f:
            ...
            for index, json_task in enumerate(taskset_package['tasks']):
                new_task = task()
                self.__apend_task(new_task.load_from_json(json_task))
    ...
    def save_taskset_to_json(self, file_name = None):
    ...
    def get_high_util(self):
        ...
        if self.task_properties["high_util"] is None:
            high_util =  self.__calculare_total_utilization('high')
        else:
    ...
    def get_taskset_high_crit_low_prio(self, task):
        ...
        comparison_op = lambda x, y: ((x.crit < y.crit) and (x.prio < y.prio))
        return self.__get_conditional_taskset(task, comparison_op)

    def get_taskset_high_crit_high_prio(self, task, taskset):
        """get tasks of higher criticality, higher priority"""
        comparison_op = lambda x, y: ((x.crit < y.crit) and (x.prio < y.prio))
        return self.__get_conditional_taskset(task, comparison_op)

    def get_taskset_low_crit_high_prio(self, task, taskset):
        """get tasks of lower criticality, higher priority"""
        comparison_op = lambda x, y: ((x.crit > y.crit) and (x.prio < y.prio))
        return self.__get_conditional_taskset(task, comparison_op)

    def get_taskset_high_crit_same_prio(self, task, taskset):
    ...
    def get_tasks_by_crit(self, flag='high'):
        ...
        condition = None
        if flag == 'high':
            condition = lambda x: return x.crit == 1
        elif flag == 'low':
            condition = lambda x: return x.crit == 0
        else:
            raise ValueError("Invalid flag specified for filtering tasks: use high or low.")
    ...
    def calculate_rta(self,task):
        """Do response time analysis for given taskset."""
        pass
